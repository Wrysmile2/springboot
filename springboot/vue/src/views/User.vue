...
<script>
 export default {
   ...
   methods:{
     ...
     save(){
       this.axios.post("http://localhost:9090/user",this.form).then(res => {
         if (res){
     ...
     handleDelete(id){
       this.axios.delete("http://localhost:9090/user/"+id).then(res => {
         if (res){
     ...
     deleteBatch(){
       ...
       this.axios.post("http://localhost:9090/user/delete/batch",ids).then(res => {
         if (res){
           this.$message.success("删除成功")
           this.load()
         }else {
